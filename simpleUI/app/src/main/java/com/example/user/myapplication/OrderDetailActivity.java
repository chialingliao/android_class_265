...
public class OrderDetailActivity extends AppCompatActivity implements GeoCodingTaskResponse, RoutingListener, GoogleApiClient.ConnectionCallbacks,
    ...
    @Override
    public void reponseWithGeoCodingResults(LatLng location) {
...
//            Toast.makeText(getApplicationContext(),"Route "+ (i+1) +": distance - "+ routes.get(i).getDistanceValue()+": duration - "+ routes.get(i).getDurationValue(),Toast.LENGTH_SHORT).show();
    ...
    public void onConnected(Bundle bundle) {
        ...
        LocationServices.FusedLocationApi.requestLocationUpdates(mGoogleApiClient, LocationRequest.create(), (LocationListener)this);


        //取得現在位置 應交由MAINACTIVITY處理
        LatLng start = new LatLng(25.0186348, 121.5398379);
        if(location !=null){
            start = new LatLng(location.getLatitude(), location.getLongitude());//經緯度
    ...
    public void onLocationChanged(Location location) {
        LatLng start = new LatLng(location.getLatitude(), location.getLongitude());//經緯度
    ...
    private  static  class GeoCodingTask extends AsyncTask<String, Void, double[]>{
        ...
        protected void onPostExecute(double[] latlng) {
            if(latlng != null && geoCodingTaskResponseWeakReference.get()!=null){
                ...
                response.reponseWithGeoCodingResults(storeLocation);
            }
...
interface  GeoCodingTaskResponse{
    void reponseWithGeoCodingResults(LatLng location);
}